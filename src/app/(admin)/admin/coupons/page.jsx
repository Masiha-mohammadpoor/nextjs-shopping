                ...
                {coupons?.map((coupon, index) => {
                  return (
                    <tr key={coupon._id}>
                      ...
                      <td className="table__td  whitespace-nowrap truncate">
                        <div className="badge bg-blue-900 flex justify-center">{coupon?.type}</div>
                        
                      </td>
                      <td className="table__td  whitespace-nowrap truncate">
                        {coupon.type === "percent" ? `${toPersianDigits(coupon.amount)} %` : toPersianNumberWithCommas(coupon.amount)}
                      </td>
                      <td className="table__td">
                        <div className="flex flex-col gap-y-2 items-start">
                          {coupon?.productIds.map((coupon, index) => {
                            return (
                      ...
                      <td className="table__td  whitespace-nowrap truncate">
                        <div className="flex gap-x-2">
                          <button><Link href={`/admin/coupons/edit/${coupon._id}`}><BiEdit className="text-lg text-success"/></Link></button>
