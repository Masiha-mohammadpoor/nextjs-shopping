...
const ProductPage = async ({params}) => {  
  ...
  const {title , imageLink , brand , category , likes , rating , price , discount , offPrice} = product || {};
      ...
      <article className="flex flex-col md:flex-row gap-x-10 gap-y-4 col-span-3 md:col-span-2">
        ...
        <div className="w-full">
          <div className="flex flex-col gap-y-3">
            ...
            <p className="flex items-center text-warning"><TbClockStar/> به همراه ضمانت</p>
            <div className="flex items-end gap-x-2 w-fit h-6 p-0 mt-5">
              <span className="text-error text-2xl">{toPersianDigits(likes?.length)}</span>
