  ...
  return (
    <form onSubmit={onSubmit} className="w-full flex flex-col">
      <div className="w-full flex flex-col items-center">
        <label
          htmlFor={name}
          className="w-full text--white mb-3 flex justify-between text-sm"
        >
          <span>{label}</span>
          <span>
            <button
            ...
            >
              ویرایش شماره موبایل <CiEdit className="mr-2" />
            </button>
          </span>
        </label>
      ...
      {time > 0 ? (
        <p className="flex justify-center mt-4 text--white">
          <span className="inline-block w-5 ml-1">
            {" "}
      ...
      <div className="w-full flex justify-start">
        ...
        >
          {checkOtpLoading ? <Loading size={10} /> : "ورود"}
        </button>
      </div>
    </form>
  );
